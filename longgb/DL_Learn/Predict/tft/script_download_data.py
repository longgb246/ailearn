...
def download_volatility(config):
    ...
    csv_path = os.path.join(
        data_folder, 'oxfordmanrealizedvolatilityindices.csv')
    zip_path = os.path.join(
        data_folder, 'oxfordmanrealizedvolatilityindices.zip')
...
def download_traffic(config):
    ...
    def read_matrix(filename):
        ...
        with open(os.path.join(data_folder, filename), 'r') as dat:
            ...
            for i, line in enumerate(lines):
                ...
                array = [
                    process_list(row_split, variable_type=float,
                                 delimiter=None)
    ...
    for store in store_columns:
        ...
        key = sliced['id'].apply(str) \
            ...
            + sliced['time_on_day'].apply(lambda x: '_' +
                                          format_index_string(x))
        ...
        flat_df = flat_df.append(
            sliced.dropna(), ignore_index=True, sort=False)
...
if __name__ == '__main__':
    ...
    # python -m script_download_data traffic F:/Codes/data1/tft_outputs
